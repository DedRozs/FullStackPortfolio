...
def test_retrieve_ai_services():
    ...
    AIService.objects.create(
        name="AI Service 1", description="Description 1", owner=test_user
    )
    AIService.objects.create(
        name="AI Service 2", description="Description 2", owner=test_user
    )

    # Check that objects were saved
    assert (
        AIService.objects.count() == 2
    ), f"Expected 2 AI services in DB, found {AIService.objects.count()}"
...
def test_update_ai_service():
    ...
    ai_service = AIService.objects.create(
        name="Old Name", description="Old Description", owner=test_user
    )
...
def test_unauthorized_update_ai_service():
    ...
    ai_service = AIService.objects.create(
        name="Owner's AI", description="Owned by user1", owner=owner_user
    )
    ...
    url = reverse("ai_service_detail", kwargs={"pk": ai_service.id})
    data = {"name": "Hacked AI Service", "description": "Unauthorized edit"}
...
def test_unauthorized_delete_ai_service():
    ...
    # Create two users
    owner_user = User.objects.create_user(
        username="owner", password="password", email="user1@example.com"
    )
    another_user = User.objects.create_user(
        username="hacker", password="password", email="user2@example.com"
    )
